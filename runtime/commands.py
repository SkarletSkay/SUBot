...
class CommandsBase:
    ...
    def send_message(self, user_id: int = None, text: typing.Optional[str] = None,
                     parse_mode: telegram.ParseMode = None, resource: typing.Optional[str] = None,
                     reply_markup: telegram.ReplyMarkup = None) -> CommandResult:
        if resource is not None:
            text = self.resources.get_string(resource)
        if user_id is None:
            user_id = self.user.id
        result = MessageResult(text, user_id, parse_mode, reply_markup)
        return result
    ...
    def send_message_list(self, messages_list: typing.Optional[typing.List[str]] = None,
                          messages_resources: typing.Optional[typing.List[str]] = None,
                          message_array: typing.Optional[str] = None,
                          parse_mode: telegram.ParseMode = telegram.ParseMode.MARKDOWN):
        ...
        result = MessageListResult(self.user.id, messages_list, parse_mode)
        return result

    def edit_message(self, message_id: int, user_id: int = None, new_text: typing.Optional[str] = None,
                     new_text_resource: typing.Optional[str] = None,
                     parse_mode: telegram.ParseMode = telegram.ParseMode.MARKDOWN,
                     new_markup: telegram.ReplyMarkup = None):
        if new_text_resource is not None:
            new_text = self.resources.get_string(new_text_resource)
        if user_id is None:
            user_id = self.user.id
        result = EditMessageResult(user_id, message_id, new_text, parse_mode, new_markup)
        return result

    def reply_to_message(self, message_id: int, user_id: int = None, text: typing.Optional[str] = None,
                         text_resource: typing.Optional[str] = None, parse_mode: telegram.ParseMode = telegram.ParseMode.MARKDOWN,
                         reply_markup: telegram.ReplyMarkup = None):
        if text_resource is not None:
            text = self.resources.get_string(text_resource)
        if user_id is None:
            user_id = self.user.id
        result = ReplyToMessageResult(message_id, user_id, text, parse_mode, reply_markup)
        return result

    def compound_result(self, results: typing.Tuple[CommandResult, ...]):
        return CompoundResult(list(results))

    def send_or_edit_message(self, message_id: int, user_id: int = None, text: str = None, text_resource: str = None,
                             parse_mode: telegram.ParseMode = telegram.ParseMode.MARKDOWN,
        ...
        if user_id is None:
            user_id = self.user.id
        return SendOrEditMessage(user_id, message_id, message_text, parse_mode, reply_markup)
...
class MessageResult(CommandResult):
    ...
    def __init__(self, message: str, chat_id: int, parse_mode: telegram.ParseMode = telegram.ParseMode.MARKDOWN,
                 reply_markup=None):
...
class ReplyToMessageResult(CommandResult):
    def __init__(self, message_id: int, chat_id: int, text: str, parse_mode: telegram.ParseMode = telegram.ParseMode.MARKDOWN, reply_markup=None):
    ...
    def execute(self, bot: telegram.Bot):
        bot.send_message(self.__chat_id, self.__text, reply_to_message_id=self.__message_id, reply_markup=self.__markup, parse_mode=self.__parse_mode)
...
class MessageListResult(CommandResult):
    def __init__(self, chat_id: int, messages: typing.List[str], parse_mode: telegram.ParseMode = telegram.ParseMode.MARKDOWN):
        self.__chat_id = chat_id
    ...
    def execute(self, bot: telegram.Bot):
        for message in self.__messages:
            bot.send_message(self.__chat_id, message, parse_mode=self.__parse_mode)
...
class EditMessageResult(CommandResult):
    def __init__(self, chat_id: int, message_id: int, new_text: str, parse_mode: telegram.ParseMode = telegram.ParseMode.MARKDOWN,
                 new_markup: telegram.ReplyMarkup = None):
        self.__chat_id = chat_id
        self.__message_id = message_id
...
class EditMessageMarkupResult(CommandResult):
    def __init__(self, chat_id: int, message_id: int, new_markup: telegram.InlineKeyboardMarkup = None):
        self.__chat_id = chat_id
        self.__new_markup = new_markup
        self.__message_id = message_id
...
class ModelCommandBase(CommandsBase):
    ...
    def on_complete(self, model) -> typing.Union[CommandResult, str]:
        return self.send_message(text="You successfully sent")

    def on_cancel(self, model) -> CommandResult:
        return self.send_message(text="You canceled")
...
class SendOrEditMessage(CommandResult):
    ...
    def execute(self, bot: telegram.Bot):
        try:
            EditMessageResult(self.__chat, self.__message_id, self.__text, self.__parse_mode, self.__markup).execute(
                bot)
        except telegram.error.BadRequest:
            MessageResult(self.__text, self.__chat, self.__parse_mode, self.__markup).execute(bot)
